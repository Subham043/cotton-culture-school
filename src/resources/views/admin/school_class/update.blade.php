...
<div class="page-content">
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-lg-12">
                <form id="countryForm" method="post" action="{{route('school_class.update.post', [$school_id, $data->id])}}" enctype="multipart/form-data">
                @csrf
                    <div class="card">
            ...
            @foreach($classes as $val)
                {
                    value: '{{$val->id}}',
                    label: '{{$val->name}}',
                    selected: {{($data->class_id==$val->id) ? 'true' : 'false'}},
                },
            @endforeach
        ],
            ...
            @foreach($sections as $section)
                {
                    value: '{{$section->id}}',
                    label: '{{$section->name}}',
                    selected: {{ (in_array($section->id, $section_data)) ? 'true' : 'false'}}
                },
            @endforeach
        ],
