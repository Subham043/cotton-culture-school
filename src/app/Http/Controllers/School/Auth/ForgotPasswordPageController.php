...
class ForgotPasswordPageController extends Controller
{
    public function index(){
        return view('school.auth.forgot_password');
    }
    ...
    public function requestForgotPassword(Request $request) {
        ...
        if($status === Password::RESET_LINK_SENT){
            ...
            return redirect(route('school_forgot_password'))->with(['success_status' => __($status)]);
        }
        return redirect(route('school_forgot_password'))->with(['error_status' => __($status)]);
