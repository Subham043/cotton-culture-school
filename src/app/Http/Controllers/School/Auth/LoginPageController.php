...
class LoginPageController extends Controller
{
    ...
    public function authenticate(Request $request){
        ...
        $credentials['role'] = Role::SCHOOL->value;

        if (Auth::attempt($credentials)) {
            $request->session()->regenerate();
            (new RateLimitService($request))->clearRateLimit();
            return redirect()->intended(route('school_dashboard'))->with('success_status', 'Logged in successfully.');
        }

        return redirect(route('school_signin'))->with('error_status', 'Oops! You have entered invalid credentials');
    }
}
