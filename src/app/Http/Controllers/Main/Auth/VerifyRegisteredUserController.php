...
class VerifyRegisteredUserController extends Controller
{
    public function index(Request $request){
        if($request->user()->hasVerifiedEmail()){
            return redirect()->intended(route('dashboard'))->with('success_status', 'Oops! you are already a verified user.');
        }
        return view('admin.pages.auth.verify_user')->with('breadcrumb','Verify Email');
    }
